...
public struct TokenResult<Token: TokenProtocol> {
    let token: Token?
...
extension TokenResult {
    ...
    public func map<T: TokenProtocol>(_ transform: (Token) throws -> T?) rethrows -> TokenResult<T> {
        return TokenResult<T>(token: try token.flatMap(transform), remainingString: remainingString)
    }
